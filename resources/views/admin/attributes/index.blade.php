                                        ...
                                        <tbody>
                                            @foreach ($attributeTypes as $index => $attributeType)
                                                <tr>
                                                                    ...
                                                                    <div class="attribute-value">
                                                                        @if (!empty($value->hex) && $value->hex[0] !== '')
                                                                            <div class="color-preview"
                                                                                style="background-color: {{ $value->hex[0] }}">
                                                                            </div>
                                                                            <span
                                                                                class="small">{{ $value->hex[0] }}</span>
                                                                        @else
                                                    ...
                                                    <td>
                                                        <span
                                                            class="badge {{ $attributeType->status == 'active' ? 'bg-success' : 'bg-danger' }}">
                                                            {{ $attributeType->status == 'active' ? 'Hoạt động' : 'Không hoạt động' }}
                                                        </span>
                                                    </td>
                                                    <td>
                                                        <a href="{{ route('admin.attributes.edit', $attributeType) }}"
                                                            class="btn btn-warning btn-sm rounded-3 me-2"><i
                                                                class="ti ti-edit"></i></a>
                                                        <form
                                                            action="{{ route('admin.attributes.destroy', $attributeType) }}"
                                                            method="POST" class="d-inline"
                                                            onsubmit="return confirm('Bạn có chắc chắn muốn xóa loại thuộc tính này không??');">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                                                                class="btn btn-danger btn-sm rounded-3"><i
