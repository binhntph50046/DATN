            ...
            <div class="main-image mb-4">
              @php
                ...
                foreach($product->variants as $variant) {
                  ...
                  foreach($imgs as $img) {
                    if (!in_array($img, $allImages)) $allImages[] = $img;
                  }
                }
                $defaultPrice = $defaultVariant ? $defaultVariant->selling_price : $product->variants->min('selling_price');
              @endphp
              <img src="{{ asset($mainImage) }}" class="img-fluid" alt="{{ $product->name }}" id="mainProductImage">
            </div>
        ...
        <div class="col-lg-6">
          <div class="product-info">
            ...
            <div class="product-price mb-4">
              <span class="current-price" id="productPrice">{{ number_format($defaultPrice, 0, ',', '.') }} VNĐ</span>
            </div>
            ...
            @foreach($attributeGroups as $typeName => $group)
              @php
                ...
                foreach($group as $item) {
                  if (!empty($item['hexes'][0])) { $hasHex = true; break; }
                }
              ...
              <div class="variant-group mb-4">
                <label class="form-label">
                  {{ ucfirst($typeName) }}
                  @if($hasHex)
                    : <span id="selected-{{ $typeName }}-value" class="ms-2 badge bg-light text-dark border">{{ $defaultValue }}</span>
                  @endif
                </label>
                <div class="variant-options mt-2 d-flex gap-3">
                  @if($hasHex)
                    @foreach($group as $item)
                      @php $value = $item['values'][0] ?? ''; $hex = $item['hexes'][0] ?? null; @endphp
                  ...
                  @else
                    ...
                    @foreach($uniqueItems as $item)
                      @php
                        ...
                        if ($defaultVariant) {
                          foreach ($defaultVariant->combinations as $comb) {
                            if (($comb->attributeValue->attributeType->name ?? '') === $typeName && ($comb->attributeValue->value[0] ?? '') === $value) {
                              $isActive = true;
                              break;
                            }
                          }
                      ...
                      @endphp
                      <button type="button"
                              class="storage-btn {{ $isActive ? 'active' : '' }}"
                              data-variant-id="{{ $item['variant_id'] }}"
                              data-attr-type="{{ $typeName }}"
                              onclick="selectVariant({{ $item['variant_id'] }}, '{{ addslashes($value) }}', '{{ $typeName }}', this)">
                        {{ $value }}
            ...
            @if($product->specifications && $product->specifications->isNotEmpty())
              <div class="specifications-section mt-4">
                ...
                @foreach($product->specifications as $spec)
                  <div class="spec-item">
                    <strong>{{ $spec->specification->name }}:</strong>
  ...
  <script>
    ...
    function updateThumbnails(images) {
      ...
      images.forEach(img => {
        ...
        div.className = 'col-3';
        div.innerHTML = `<img src=\"{{ asset('') }}${img}\" class=\"img-fluid thumbnail\" alt=\"Thumbnail\" onclick=\"changeMainImage(this.src)\">`;
