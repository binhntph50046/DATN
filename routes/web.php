...
Route::prefix('admin')
    ...
    Route::post('/users/toggle-status/{user}', [UserController::class, 'toggleStatus'])->name('users.toggle-status');
        ...
        ; // Bạn thay đổi permission nếu có
        Route::get('ajax/product-variants/{id}', [FlashSaleController::class, 'getVariantsByProduct']);
        Route::post('flash-sales/{id}/return-stock', [FlashSaleController::class, 'returnStock'])
    ->name('flash-sales.return-stock');
    // Blog Routes
    ...
    Route::put('blogs/{id}/restore', [BlogController::class, 'restore'])->middleware('permission:edit blogs')->name('blogs.restore');
        ...
        Route::resource('flash-sale-items', FlashSaleItemController::class)
            // ->middleware('permission:view flash sale items')
        ; // Nếu có phân quyền riêng
